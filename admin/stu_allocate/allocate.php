...
<div class="wrapper">
	...
	<div class="box">
        ...
        <div class="boxinner">
        	<p>You are here &raquo; <strong><!-- InstanceBeginEditable name="here" --><a href="index.php?flag=non">Home</a> &raquo; Allocate Student<!-- InstanceEndEditable --></strong></p>
      </div>
    </div>
...
if($id > 0)
{
	...
	?>
        ...
        <!-- Start Here -->
			...
			<h2>Students Allocation To Teacher</h2>
		 ...
		 <form action="submit.php" method="POST"  name="regisfrm" onsubmit='return admin_ward();' class="form">		
            	...
            	<table width="66%" border="0" class="tbl" align="center">
                  ...
                  <tbody class='ms'>
				   ...
				   $table_name = 's_'.$id.'_a'; 
				   $table_name1 = 's_'.$id.'_m';
				   $que_tab="SELECT * FROM ".$table_name." LIMIT 1";
					if(!($check_tab = $con->query($que_tab))){
						 $query_create=sprintf("CREATE TABLE IF NOT EXISTS ".$table_name."(`id` int(11) NOT NULL AUTO_INCREMENT,`stud_id` int(11) NOT NULL,PRIMARY KEY (`id`),`faculty_id` int(11), UNIQUE KEY `stud_id` (`stud_id`)) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1");
						 if(!($result_create = $con->query($query_create))){echo $con->error; exit;}
						 $query_create1=sprintf("CREATE TABLE IF NOT EXISTS ".$table_name1."(`id` int(11) NOT NULL AUTO_INCREMENT,`stud_id` int(11) NOT NULL,PRIMARY KEY (`id`),`faculty_id` int(11), UNIQUE KEY `stud_id` (`stud_id`)) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1");	
						if(!($result_create = $con->query($query_create1))){echo $con->error; exit;}	
					}
					...
					if($rowCk>0){
						...
						$que="SELECT * FROM stu_provisional s LEFT JOIN ".$table_name." a ON s.prov_id=a.stud_id WHERE s.prov_id NOT IN (SELECT stud_id from ".$table_name.") and s.status=1 $condition2";
					}else{
						$condition2=" ORDER BY prov_id ASC";	
				   ...
				   ?>
                    ...
                    <?php   } ?>
   					...
   					<input name="txtmode" type="hidden" value="allocate" />
                    <input name="t_sub_id" type="hidden" value="<?=$id?>" />
                    <input name="t_all_id" type="hidden" value="<?=$tid?>" />
			<?php if($rowCount>0){?><input name="input" type="submit" value="Submit" class="button"/>
			<?php }else{ ?><p align="center"><font color='red'><B>ALL STUDENTS HAVE BEEN ALLOCATED.</B></font></p> <a href="index.php?flag=non" class="button">Back</a><?php }?>
		  </form>
		</div>
