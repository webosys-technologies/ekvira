...
if($mode=="del")
{
	...
	$query=sprintf("DELETE FROM stu_city WHERE city_id ='%d'", $id);
...
if($id=="")
{
	$query_add=sprintf("INSERT INTO stu_city SET city='%s'", $name);	
...
if($id!="")
{	
	$qadd=sprintf("UPDATE stu_city SET city='%s' WHERE city_id='%d'", $name, $id);		
