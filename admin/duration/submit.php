...
if($mode=="del")
{
	...
	$query=sprintf("DELETE FROM stu_duration WHERE d_id ='%d'", $id);
...
if($id=="")
{
	$query_add=sprintf("INSERT INTO stu_duration SET duration='%s'", $name);	
...
if($id!="")
{	
	$qadd=sprintf("UPDATE stu_duration SET duration='%s' WHERE d_id='%d'", $name, $id);		
