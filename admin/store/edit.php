...
<div class="wrapper">
	...
	<div class="box">
		...
		?><a href="index.php?stock=<?=$stock_name?>&flag=non">Home</a> &raquo; Edit Store Item<!-- InstanceEndEditable --></strong></p>
      </div>
    </div>
...
{
	if($stock_name=='study'){
		$query=sprintf("SELECT * FROM stu_study_stock WHERE id='%d'", $id); 
		if(!($result = $con->query($query))){echo $con->error; exit;}
		$row_add=$result->fetch_assoc();
	}else{
		$query=sprintf("SELECT * FROM stu_physical_stock WHERE id='%d'", $id); 
		if(!($result = $con->query($query))){echo $con->error; exit;}
		$row_add=$result->fetch_assoc();
	}		
}
...
?>
    ...
    <div class="main-wrapper">
        <!-- Start Here -->
			...
			<h2><?php echo $title;?>  Store Item Details</h2>
			...
			<div class="field">
				...
				<div class="fieldrt">
				  <input type="text" class="input-text" value="<?php echo stripslashes($row_add['item_name']);?>" title="Store Item Name" id="name" name="name" onblur='notnull(this.id);' />
				</div>
			</div>
            <?php
			if($stock_name=='study'){
            ...
            <div class="field">
				...
				<div class="fieldrt">
				<input type="text" class="input-text" title="Publication Name"  value="<?php echo stripslashes($row_add['publication']);?>" id="p_name" name="p_name" onBlur='notnull(this.id);' />
				</div>
			</div>
        ...
        <div class="field">
				...
				<div class="fieldrt">
					<input type="text" class="input-text" title="Brand Name"  value="<?php echo stripslashes($row_add['brand']);?>" id="p_name" name="p_name" onBlur='notnull(this.id);' />
				</div>
			</div>
            <?php }
			?>
            <div class="field">
				...
				<div class="fieldrt">
					<input type="text" class="input-text" title="Store Item Qty"  value="<?php echo stripslashes($row_add['qty']);?>" id="item_qty" name="item_qty" onBlur='notnull(this.id);' />
				</div>
			</div>
            ...
            <div class="field">
				...
				<div class="fieldrt">
					<input type="text" class="input-text" title="Other"  value="<?php echo stripslashes($row_add['other']);?>" id="other" name="other" onBlur='notnull(this.id);' />
				</div>
			</div>
