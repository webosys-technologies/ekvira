...
<div class="wrapper">
	...
	<div class="box">
        ...
        <div class="boxinner">
        	<p>You are here &raquo; <strong><!-- InstanceBeginEditable name="here" --><a href="list-enquiry.php?flag=non">Home</a> &raquo;Renew Admission<!-- InstanceEndEditable --></strong></p>
      </div>
    </div>
...
$(document).ready(function()
{
	...
	$("#state").change(function()
	{
     ...
     <?php
		...
		if($id > 0)
		{
			$query=sprintf("SELECT * FROM stu_cancel_prov WHERE cancel_id='%d'", $id); 
	...
	?>
        ...
        <h2>renew cancelled/ course completed Student</h2>
        <form action="submit.php?flag=<?=$flag?>" method="post" class="form">
        ...
        <div class="field">
            ...
            <div class="fieldrt">
                <input type="text" class="input-text" value="<?=$row_add['dob']?>" title="DOB" id="dob"  name="dob" required />
            </div>
        </div>
        ...
        <div class="field">
            ...
            <div class="fieldrt">
                ...
                <select name="country" id="country" class="slct" required >
                  <option selected="selected" value="">Please Select Country</option>
						...
						{
						?>
                        <option value='<?php echo $row_country['country_id']?>' <?php if($row_add['country_id']==$row_country['country_id']){?>selected="selected"<?php }?> ><?php echo stripslashes($row_country['country_name']);?></option> 
				  <?php } ?>
         ...
         <div class="field">
            ...
            <div class="fieldrt">
                <select name="state" id="state" class="slct" required >
                  <option selected="selected" value="">Please Select State</option>
						...
						{
						?>
                        <option value='<?php echo $row_state['state_id']?>' <?php if($row_add['state_id']==$row_state['state_id']){?>selected="selected"<?php }?> ><?php echo stripslashes($row_state['state_name']);?></option> 
				  <?php } ?>
            ...
            <div class="fieldrt">
                <select name="city" id="city" class="slct"  >
                  <?php
						...
						{
						?>
                        <option value='<?php echo $row_city['city_id']?>' <?php if($row_add['city_id']==$row_city['city_id']){?>selected="selected"<?php }?> ><?php echo stripslashes($row_city['city_name']);?></option> 
				  <?php } ?>
                </select>
            </div>
        </div>
  ...
  <TABLE id="courseTable"  border="0">
	<?php
    $query_c_course = sprintf("SELECT * FROM stu_course c,stu_cancel_course e where c.course_id=e.course_id and e.cancel_id='%d'", $id);
    if (!($result_c_course = $con->query($query_c_course))) 
    { echo "FOR QUERY: $query_c_course<BR>".$con->error; 	exit;}
    while($row_c_course=$result_c_course->fetch_assoc())
    {
        $course=$row_c_course['course'];
    ?>
        <TR>
			...
			{
            ?>
            <option value='<?php echo $row_course['course_id']?>' <?php if($row_c_course['course_id']==$row_course['course_id']){?>selected="selected"<?php }?> ><?php echo stripslashes($row_course['course']);?></option>
...
<?php
		$query_cancel_edu = sprintf("SELECT * FROM stu_cancel_edu WHERE cancel_id='%d'", $id);
		if (!($result_cancel_edu = $con->query($query_cancel_edu))) 
		{ echo "FOR QUERY: $query_cancel_edu<BR>".$con->error; 	exit;}
		while($row_cancel_edu=$result_cancel_edu->fetch_assoc())
		{
			$edu_id=$row_cancel_edu['edu_id'];
...
?>
        <TR>
            ...
            <TD width="100">
            	<input type="text" class="input-text" value="<?=$row_cancel_edu['percent']?>" title="Percentage" id="percent[]" name="percent[]" placeholder="percent(%)" required />                 
            </TD>
        </TR>
 <?php }?>
</TABLE>
        ...
        <div class="field">
            ...
            <div class="fieldrt">
            <ul class="halflist">
              <li><input name="c_certi" type="radio" <?php if($row_add['c_certi']=='YES'){?>checked="checked"<?php }?> value="YES"  required /><label for="male">YES</label></li>
              <li><input name="c_certi" type="radio" <?php if($row_add['c_certi']=='NO'){?>checked="checked"<?php }?> value="NO" checked="checked" style="margin-left:25%;" required /><label for="female">NO</label></li>
              </ul>
            </div>
        </div>
        ...
        <input type="hidden" value="renew" id="mode" name="mode"  required />
        <div class="field">
        <label>&nbsp;</label>
