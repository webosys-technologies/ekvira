...
<div class="wrapper">
	...
	<div class="box">
        ...
        <div class="boxinner">
        	<p>You are here &raquo; <strong><!-- InstanceBeginEditable name="here" --><a href="index.php?flag=non">Home</a> &raquo; View Student Details<!-- InstanceEndEditable --></strong>
                    <div align="right">
                    <input type="button" onclick="window.print();" class="button" value="Print &#38 PDF" align="right" />
	...
	if (!($page_res = $con->query($que))){ echo "FOR QUERY: $que<BR>".$con->error; 	exit;}
	   ...
	   ?>
        ...
        $allocated = sprintf("SELECT faculty_id FROM s_1_a WHERE stud_id='%d'", $id);
        $class = $con->query($allocated);
        $actual=$class->fetch_assoc();
//        print_r($id); die;


        $class_teacher_name = sprintf("SELECT * FROM stu_faculty WHERE id='%d'", $actual['faculty_id']);
        $ctn = $con->query($class_teacher_name);
        $class_teacher=$ctn->fetch_assoc();
        
//        print_r($class_teacher); die;
...
//        die;
     ...
     <div class="main-wrapper">
       ...
       <div class="row" id="result">
            ...
            <div class="stu-details-show-left">
                <table width="50%">
                    ...
                    <tr><td><h5 align="left" style="font-weight: bold;">Mothers Name:</h5></td><td><h5 align="left"><? echo $mother?></h5></td>
                        ...
                        <tr><td><h5 align="left" style="font-weight: bold;">Classteacher's Name:</h5></td><td><h5 align="left"><?php echo $class_teacher['name']; ?></h5></td></tr>
                </table>
            </div></td><td width="50%">
        ...
        <!--                here                              -->
        <h4 align="center" style="color:#337ab7;float:left;">
   ...
   <?php
    $print_data.='
        ...
        <div>
            ...
            <img src=\"vmca/pages/teacher/marks/onlylogo.png\" height="150px" width="150px">
        </div>
...
//                            print_r($mark_data);
                   ...
                   //        [exam_date] => 2016-12-08
                    ...
                    <table class="table table-hover" colspan="4">
<!--                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            $subject_master=sprintf("SELECT * FROM stu_term_marks where subject='%d' and stu_id='%d' group by skill",$subject,$mark_data['stu_id']); 
                            $page_res_subject = $con->query($subject_master);
                            ...
                            $print_data.='
                            <tr>
                                <th align="left" width="15%"><h5 style="color:blue;">'.$skillname.':</h5></th>';?>
                            <tr>
                                <th><? echo $skillname?></th>
                                <?php
                                //****** Get subject term data
                                
                                $level_master=sprintf("SELECT * FROM stu_term_marks where subject='%d' and stu_id='%d' and skill='%d'",$subject,$mark_data['stu_id'],$skill); 
                                ...
                                while($level_data=$page_res_level->fetch_assoc())
                                {
                                    ...
                                    $total_marks+=$level_data2['marks'];
                                    $top_mark+=5;
                                    $result = $level_data2['result'];
                                    echo '<td>'.$level_name.'&nbsp; &nbsp;<strong>'.$result.'</strong></td>';
                                $print_data.='
                                <td align="left" width="25%"><h6 style="color:black;">'.$level_name.'&nbsp; &nbsp;<strong style="color:green;">'.$result.'</strong></h6></td>
                               ';?>
                                <?php
...
<!-- Main Body End -->
                ...
                <?php
                    //echo $total_marks.' and '.$total_sub.' and'.$top_mark;
                    $avg_result=($total_marks/$top_mark)*100;
                    if($mark_data['term']==1)
                    {
                        $query_fees = sprintf(" UPDATE stu_provisional SET term1='%f' WHERE prov_id='%d' ",$avg_result, $id);
                        if (!($result_fees = $con->query($query_fees))){ echo "FOR QUERY: $query_fees<BR>".$con->error;}
                    }
                    else
                    {
                        $query_fees = sprintf(" UPDATE stu_provisional SET term2='%f' WHERE prov_id='%d' ",$avg_result, $id);
                        if (!($result_fees = $con->query($query_fees))){ echo "FOR QUERY: $query_fees<BR>".$con->error;}
                    }
                ?>
<script>
