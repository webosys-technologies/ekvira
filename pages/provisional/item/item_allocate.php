...
function toggle_study(source) {
  checkboxes = document.getElementsByName('items_study[]');
  for(var i=0, n=checkboxes.length;i<n;i++) {
    checkboxes[i].checked = source.checked;
  }
}
function toggle_phy(source) {
  checkboxes = document.getElementsByName('items_phy[]');
  for(var i=0, n=checkboxes.length;i<n;i++) {
    checkboxes[i].checked = source.checked;
  }
}
...
<div class="wrapper">
	...
	<div class="box">
        ...
        <div class="boxinner">
        	<p>You are here &raquo; <strong><!-- InstanceBeginEditable name="here" --><a href="index.php?flag=non">Home</a> &raquo; Allocate Store Items<!-- InstanceEndEditable --></strong></p>
      </div>
    </div>
    ...
    <div class="fullbox">
	   ...
	   ?> 
            ...
            <? if($flag=='add'){ if($row_prov['photo_url']==''){ ?>
             <p align="center"><font color='red'><B>Image is not Uploaded</B></font></p>
            <?php }else{ ?>
            <p align="center"><font color='red'><B>Image has been Uploaded Successfully</B></font></p>
            <? } }?>
             
        <div class="main-wrapper">
        <!-- Start Here -->
       <p align="center" class="style1"><strong>ALLOCATE STORE ITEMS</strong></p>
   ...
   <form action="submit.php" method="post">
        ...
        <table width="80%" height="420" border="1" class="tbl" align="center">
          <tr>
            <td height="51"><strong>Student Name :--              </strong></td>
            <td height="51" colspan="2"><?=$fullname?></td>
            </tr>
           <tr>
             <td width="28%"><strong>Study Stock Items :-</strong></td>
...
<?php
		  $condition2=" ORDER BY id DESC";	
				...
				{ 
					...
					$query_study = sprintf("SELECT * FROM stu_prov_item WHERE prov_id='%d' and stock_id=1 and item_id='%d' and return_date IS NULL", $id, $item_id);
 			...
 			?>
					<tr><td> 
					<input class="checkbox" name="items_study[]" type="checkbox" value="<?=$row_store['id']?>" <?php if($remain_study==0){?> disabled="disabled" <?php } ?> />   
				   </td>
                <td><div align="center">
                  <?=$row_store['item_name']?>
                </div></td>
              ...
              <?php }  } ?>
          		...
          		<?php if($having!=1){ ?>
						<input type="hidden" name="items_study[]" value="" />
                       <font color="#FF0000"><B>All Study Items Are Allocated</b></font> 
			  	<?php		
			  		  }  
                ?>           </td>
            </tr>
          ...
          <tr>
            <td><strong>Physical Stock Items:-</strong></td>
            <td height="94" colspan="2">
              ...
              <table width="90%" border="0" class="tbl" align="center">
                <thead>
                  <tr>
                    ...
                    <p> All </p>
                    <p>
                      <input name="checkallphy" type="checkbox" onClick="toggle_phy(this)" />
                    </p>                      </th>
                    <th width="30%">Item Name</th>
                    <th width="17%">Remaining Quantity</th>
		  ...
		  $condition2=" ORDER BY id DESC";	
				...
				{ 
					$item_id=$row_store['id'];
					$tot_qty=$row_store['qty'];
					$que_phy_qty = sprintf("SELECT * FROM stu_prov_item WHERE stock_id=2 and item_id='%d' and return_date IS NOT NULL", $item_id);
					if (!($result_phy_qty = $con->query($que_phy_qty))) 
					{ echo "FOR QUERY: $que_phy_qty<BR>".$con->error; 	exit;}
					$phy_count = $con->affected_rows;
					$remain_phy = $tot_qty-$phy_count;
					
					$query_phy = sprintf("SELECT * FROM stu_prov_item WHERE prov_id='%d' and stock_id=2 and item_id='%d' and return_date IS NULL", $id, $item_id);
					if (!($result_phy = $con->query($query_phy))) 
					...
					if(	$rowCount2==0)
					{
					 $having=1;			
 			?>
					   <tr>	<td><input class="phy_checkbox" name="items_phy[]" type="checkbox" value="<?=$row_store['id']?>" <?php if($remain_phy==0){?> disabled="disabled" <?php } ?> /></td>
                        <td><div align="center">
                          <?=$row_store['item_name']?>
                        </div></td>
                        <td><div align="center">
              ...
              <?php }  } ?>
       		...
       		<?php if($having!=1){ ?>
						<input type="hidden" name="items_phy[]" value="" />
                        <font color="#FF0000"><B>All Physical Training Items Are Allocated</b></font>
			  	<?php		
			  		  }  
                ?>				   </td>
            </tr>
