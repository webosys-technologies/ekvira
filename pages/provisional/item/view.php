...
<div class="wrapper">
	...
	<div class="box">
        ...
        <div class="boxinner">
        	<p>You are here &raquo; <strong><!-- InstanceBeginEditable name="here" --><a href="index.php?flag=non">Home</a> &raquo; Stock Item Allocation List<!-- InstanceEndEditable --></strong></p>
      </div>
    </div>
    ...
    <div class="fullbox">
	   ...
	   ?>
        ...
        <table width="100%" border="0" class="tbl">
          ...
          <tbody class='ms'>
				...
				if($rowCount1>0)
					...
					$i=1;
						while($row_item = $result_study->fetch_assoc())
						{
							...
							$allocate_date = $row_item['allocate_date'];
							$allocate_time = $row_item['allocate_time'];
            ...
            <tr>
              ...
              <td><?=$row_item_name['item_name']?></td>
              <td><?=$row_item_name['publication']?></td>
              <td><?=$row_item_name['other']?></td>
              <td><?=$allocate_date.' <strong>/</strong> '.$allocate_time?></td>
              </tr>
            <?php } } }else{ echo '<font color="#FF0000"><B>NO PREVIOUS ALLOCATION</b></font>'; } ?>
          </tbody>
        </table>
        <h3>Allocated Physical Item</h3>
        <table width="100%" border="0" class="tbl">
          <thead>
            <tr>
              <th width="8%">SR. NO.</th>
          ...
          <tbody class='ms'>
            <?php
				...
				if($rowCount2>0)
					...
					if (!($item_res = $con->query($que_phy))){ echo "FOR QUERY: $que_phy<BR>".$con->error; 	exit;}
					if($row_item_name=$item_res->fetch_assoc()){
        ...
        <table width="100%" border="0" class="tbl">
          ...
          <tbody class='ms'>
            <?php
					$query_stock = sprintf("SELECT * FROM stu_prov_item WHERE prov_id='%d' and return_date IS NOT NULL order by allocate_date ASC", $id);
				...
				if($rowCount3>0)
					{
					$i=1;
						while($row_item = $result_stock->fetch_assoc())
						{
							$item_id = $row_item['item_id'];
							$stock_id = $row_item['stock_id'];
							$allocate_date = $row_item['allocate_date'];
							$allocate_time = $row_item['allocate_time'];
							$return_date = $row_item['return_date'];
							$return_time = $row_item['return_time'];
...
//if($row_item_name=$item_res->fetch_assoc()){
										...
										if($stock_id==1)
										{	
											$que_study="SELECT * FROM stu_study_stock where id=$item_id";
											if (!($item_res = $con->query($que_study))){ echo "FOR QUERY: $que_study<BR>".$con->error; 	exit;}
											if($row_item_name=$item_res->fetch_assoc()){
											$item_n=$row_item_name['item_name'];
											$detail=$row_item_name['publication'];
											$other=$row_item_name['other'];	
											}					 
										}else{
											$que_study="SELECT * FROM stu_physical_stock where id=$item_id";
											if (!($item_res = $con->query($que_study))){ echo "FOR QUERY: $que_study<BR>".$con->error; 	exit;}
											if($row_item_name=$item_res->fetch_assoc()){
											$item_n=$row_item_name['item_name'];
											$detail=$row_item_name['brand'];
											$other=$row_item_name['other'];
											}
										}
            ...
            <tr>
              ...
              <td><?php if($stock_id==1){echo 'STUDY';}else{echo 'PHYSICAL';}?></td>
              <td><?=$item_n?></td>
              <td><?=$detail?></td>
              <td><?=$other?></td>
              <td><?=$allocate_date.' <strong>/</strong> '.$allocate_time?></td>
